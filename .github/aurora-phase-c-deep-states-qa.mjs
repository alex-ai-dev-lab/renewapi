...
function responseFor(url, method = 'GET') {
  ...
  if (pathname === '/api/vendors/' || pathname === '/api/vendors/search') return ok({ items: vendors, total: vendors.length, page: 1, page_size: 1000 })
  ...
  if (pathname === '/api/user/' || pathname === '/api/user') return ok(paged(users, url, 20))
  if (pathname === '/api/user/search') return ok(paged(users, url, 20))
...
async function createContext({ theme = 'light', viewport = { width: 1440, height: 1000 }, failTarget = null, emptyTarget = null, delayTarget = null } = {}) {
...
for (const scenario of errorScenarios) {
  ...
  await page.getByRole('alert').filter({ hasText: scenario.title }).waitFor({ state: 'visible', timeout: 25000 })
...
const darkMobile = await createContext({ theme: 'dark', viewport: { width: 375, height: 812 }, failTarget: 'keys' })
const darkMobilePage = await preparePage(darkMobile.context, 'keys-error-dark-mobile', '/keys')
await darkMobilePage.getByRole('alert').filter({ hasText: 'Failed to load API keys' }).waitFor({ state: 'visible', timeout: 25000 })
...
const emptyScenarios = [
  { target: 'channels', route: '/channels', text: 'No channels' },
  { target: 'keys', route: '/keys', text: 'No API Keys Found' },
  { target: 'logs', route: '/usage-logs/common', text: 'No Logs Found' },
  { target: 'models', route: '/models/metadata', text: 'No Models Found' },
  { target: 'users', route: '/users', text: 'No Users Found' },
]
...
for (const scenario of emptyScenarios) {
  ...
  await page.getByText(scenario.text, { exact: false }).first().waitFor({ state: 'visible', timeout: 15000 })
...
await keysPage.getByRole('checkbox', { name: 'Select row' }).first().click()
await keysPage.getByRole('button', { name: 'Delete selected API keys' }).waitFor({ state: 'visible' })
await screenshot(keysPage, 'keys-bulk-selection-light')
await keysPage.getByRole('button', { name: 'Delete selected API keys' }).click()
await keysPage.getByText('Delete 1 API key(s)?', { exact: false }).waitFor({ state: 'visible' })
await screenshot(keysPage, 'keys-bulk-delete-confirm-light')
await keysPage.getByRole('button', { name: 'Cancel' }).click()
await keysPage.getByRole('checkbox', { name: 'Select row' }).first().click()
...
await modelsPage.getByRole('button', { name: 'Open menu' }).first().waitFor({ state: 'visible' })
await screenshot(modelsPage, 'models-management-open-light')
await modelsPage.getByRole('button', { name: 'Open menu' }).first().click()
await modelsPage.getByRole('menuitem', { name: 'Edit' }).click()
await modelsPage.getByText('Edit Model', { exact: true }).waitFor({ state: 'visible', timeout: 10000 })
...
await modelsPage.getByText('Delete Model', { exact: true }).waitFor({ state: 'visible', timeout: 10000 })
...
const summary = {
  result: consoleErrors.length === 0 && pageErrors.length === 0 && unhandled.length === 0 ? 'passed' : 'failed',
  observations,
  consoleErrors,
  pageErrors,
  unhandled,
...
if (summary.result !== 'passed') {
  throw new Error(`Phase C QA failed: ${JSON.stringify(summary, null, 2)}`)
}
