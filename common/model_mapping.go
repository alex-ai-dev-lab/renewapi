...
// ParseModelMapping accepts both the legacy one-to-one form
// {"model":"upstream-model"} and the ordered fallback form
// {"model":["upstream-a","upstream-b"]}.
func ParseModelMapping(raw string) (map[string][]string, error) {
	...
	if err := UnmarshalJsonStr(raw, &decoded); err != nil {
		return nil, err
	}
	for source, value := range decoded {
		source = strings.TrimSpace(source)
		if source == "" {
			return nil, fmt.Errorf("model mapping source must not be empty")
		}
		...
		for _, candidate := range candidates {
			...
			if candidate == "" {
				return nil, fmt.Errorf("model mapping target for %q must not be empty", source)
			}
...
// ResolveModelMappingCandidates expands legacy chained redirects while
// retaining the configured order of fallback targets.
func ResolveModelMappingCandidates(mapping map[string][]string, source string) ([]string, error) {
	...
	expand = func(current string, path map[string]bool, depth int) error {
		...
		if !exists {
			if _, seen := seenResolved[current]; !seen {
				seenResolved[current] = struct{}{}
				resolved = append(resolved, current)
			}
			return nil
		}
		nextPath := make(map[string]bool, len(path)+1)
		for modelName, active := range path {
			nextPath[modelName] = active
		}
		nextPath[current] = true
		for _, target := range targets {
			if target == current {
				if _, seen := seenResolved[target]; !seen {
					seenResolved[target] = struct{}{}
					resolved = append(resolved, target)
				}
				continue
			}
			if err := expand(target, nextPath, depth+1); err != nil {
				return err
			}
		}
		return nil
	...
	if err := expand(source, nil, 0); err != nil {
		return nil, err
	}
	return resolved, nil
}
