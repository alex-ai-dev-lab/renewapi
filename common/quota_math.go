...
func (c *QuotaClamp) AuditMap() map[string]interface{} {
	...
	return map[string]interface{}{
		"op": c.Op, "kind": c.Kind, "original": c.Original, "clamped": c.Clamped,
	}
}
...
func saturateQuota(value float64, op string) (int, *QuotaClamp) {
	...
	case value >= MaxQuota:
		SysError(fmt.Sprintf("quota conversion (%s) overflow: %g clamped to %d", op, value, MaxQuota))
		return MaxQuota, &QuotaClamp{Op: op, Kind: QuotaClampOverflow, Original: value, Clamped: MaxQuota}
	case value <= MinQuota:
		SysError(fmt.Sprintf("quota conversion (%s) underflow: %g clamped to %d", op, value, MinQuota))
...
func QuotaFromDecimalChecked(value decimal.Decimal) (int, *QuotaClamp) {
	floatValue, _ := value.Round(0).Float64()
	return saturateQuota(floatValue, "QuotaFromDecimal")
}
