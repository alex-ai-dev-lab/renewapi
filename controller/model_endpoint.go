...
func UpdateChannelModelEndpoints(c *gin.Context) {
	...
	result, err := channelconfig.Update(channelconfig.UpdateCommand{Channel: channel, PresentFields: map[string]bool{"model_endpoints": true}, ModelEndpoints: &endpoints,
		Audit: channelconfig.AuditMetadata{OperatorID: c.GetInt("id"), RequestID: c.GetString(common.RequestIdKey), Reason: "legacy model endpoint update"}})
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"success": true, "message": "", "config_version": result.Channel.ConfigVersion, "no_op": result.NoOp, "cache_synchronized": result.CacheSynchronized})
}
