...
func verifyCreemSignature(payload string, signature string, secret string) bool {
	if secret == "" {
		logger.LogWarn(context.Background(), fmt.Sprintf("Creem webhook secret 未配置 test_mode=%t payload_bytes=%d signature_present=%t", setting.CreemTestMode, len(payload), signature != ""))
		if setting.CreemTestMode {
			logger.LogInfo(context.Background(), fmt.Sprintf("Creem webhook 验签已跳过 reason=test_mode payload_bytes=%d signature_present=%t", len(payload), signature != ""))
			return true
		}
		return false
	}
...
func RequestCreemPay(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem 支付请求已收到 user_id=%d payload_bytes=%d", c.GetInt("id"), len(bodyBytes)))
...
func CreemWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem webhook 收到请求 path=%q client_ip=%s payload_bytes=%d signature_present=%t", c.Request.RequestURI, c.ClientIP(), len(bodyBytes), signature != ""))
	if signature == "" {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 缺少签名 path=%q client_ip=%s payload_bytes=%d", c.Request.RequestURI, c.ClientIP(), len(bodyBytes)))
	...
	if !verifyCreemSignature(string(bodyBytes), signature, setting.CreemWebhookSecret) {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 验签失败 path=%q client_ip=%s payload_bytes=%d", c.Request.RequestURI, c.ClientIP(), len(bodyBytes)))
		c.AbortWithStatus(http.StatusUnauthorized)
		return
	}
	...
	if err := c.ShouldBindJSON(&webhookEvent); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem webhook 解析失败 path=%q client_ip=%s payload_bytes=%d error=%q", c.Request.RequestURI, c.ClientIP(), len(bodyBytes), err.Error()))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
...
func handleCheckoutCompleted(c *gin.Context, event *CreemWebhookEvent) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem 支付完成回调 trade_no=%s creem_order_id=%s amount_paid=%d currency=%s product_name=%q customer_email_masked=%q customer_name_present=%t", referenceId, event.Object.Order.Id, event.Object.Order.AmountPaid, event.Object.Order.Currency, event.Object.Product.Name, common.MaskEmail(event.Object.Customer.Email), event.Object.Customer.Name != ""))
...
func genCreemLink(ctx context.Context, referenceId string, product *CreemProduct, email string, username string) (string, error) {
	...
	logger.LogInfo(ctx, fmt.Sprintf("Creem 支付请求已发送 api_url=%s product_id=%s email_masked=%q trade_no=%s", apiUrl, product.ProductId, common.MaskEmail(email), referenceId))
	...
	logger.LogInfo(ctx, fmt.Sprintf("Creem API 响应已收到 trade_no=%s status_code=%d payload_bytes=%d", referenceId, resp.StatusCode, len(body)))
	...
	logger.LogInfo(ctx, fmt.Sprintf("Creem 支付链接创建成功 trade_no=%s response_id=%s checkout_url_present=%t", referenceId, checkoutResp.Id, checkoutResp.CheckoutUrl != ""))
