...
- `PublicRelayError` 统一对外错误；仅本地生成的用户拒绝保留原语义，上游同名 code 不能绕过脱敏。管理员真实错误及逐渠道尝试链保存在既有 `Other.admin_info`，凭证仍须清理；用户日志读取同时清理历史记录。
- 每次失败独立进入健康记录，最终成功不抹除前序失败。健康记录同步落库后才完成当前尝试，避免后台写入与最终结果顺序不一致。
