...
func GeminiChatStreamHandlerWithCompletionGuard(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	usage, err := geminiStreamHandlerWithCompletionGuard(c, info, resp, func(data string, geminiResponse *dto.GeminiChatResponse) bool {
		multiCandidateOpenAI := info.RelayFormat == types.RelayFormatOpenAI && expectedCandidates > 1
		var stoppedChoices map[int]struct{}
		if multiCandidateOpenAI {
			stoppedChoices = make(map[int]struct{}, len(geminiResponse.Candidates))
			for _, candidate := range geminiResponse.Candidates {
				if candidate.FinishReason != nil && *candidate.FinishReason == "STOP" {
					stoppedChoices[int(candidate.Index)] = struct{}{}
				}
			}
		}
		...
		if multiCandidateOpenAI {
			for choiceIdx := range response.Choices {
				...
				// A candidate with a function call must finish as tool_calls even
				// when Gemini reports STOP for the same candidate.
				if len(choice.Delta.ToolCalls) > 0 {
					choice.FinishReason = &constant.FinishReasonToolCalls
					continue
				}
				if _, ok := stoppedChoices[choice.Index]; ok {
