...
// type-erased io.Reader prevents net/http from deriving either ContentLength
// or a replay reader for transparent HTTP/2 retries.
func NewOutboundJSONBody(data []byte) (body io.Reader, size int64, getBody func() (io.ReadCloser, error), closer io.Closer, err error) {
	...
	return common.ReaderOnly(storage), storage.Size(), func() (io.ReadCloser, error) {
		return common.OpenBodyStorageReader(storage)
	}, storage, nil
}
