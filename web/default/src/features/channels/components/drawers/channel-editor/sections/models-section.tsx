...
type PrefillGroup = { id: number; name: string; items: string | string[] }
...
type ChannelModelsEditorSectionProps = {
  form: UseFormReturn<ChannelFormValues>
  currentType: number
  currentModels: string[]
  modelOptions: SelectOption[]
  groupOptions: SelectOption[]
  prefillGroups: PrefillGroup[]
  mappingGuardrail: ModelMappingGuardrail
  mappingPreviewPairs: MappingPair[]
  ...
  onCopyModels: () => void
  onClearModels: () => void
  onAddPrefillGroup: (group: PrefillGroup) => void
  onUpdateModels: (models: string[]) => void
}

const formatModelNames = (models: string[]) =>
  models.map((model) => `"${model}"`).join(', ')

export function ChannelModelsEditorSection(
  props: ChannelModelsEditorSectionProps
) {
  const { t } = useTranslation()

  return (
    <ChannelModelsSection id={getChannelEditorSection('models').anchorId}>
              ...
              <FormItem className='space-y-3'>
                <div className='flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between'>
                  ...
                  <Badge variant='outline' className='w-fit'>
                    {t('Selected {{count}}', {
                      count: props.currentModels.length,
                    })}
                  </Badge>
                </div>
                <FormControl>
          ...
          <div className='space-y-3'>
            ...
            <div className='flex flex-wrap gap-2'>
              <Button
                ...
                disabled={!props.hasRelatedModels}
              >
              ...
              <Button
                ...
                disabled={props.currentModels.length === 0}
              >
              ...
              <FormItem className='space-y-3'>
                ...
                <FormControl>
                  <ModelMappingEditor
                    ...
                    sourceModelOptions={props.currentModels}
                    targetModelOptions={props.modelOptions.map(
                      (option) => option.value
                    )}
                  />
                </FormControl>
                {props.mappingGuardrail.invalidJson ? (
                ...
                {props.mappingGuardrail.missingSourceModels.length > 0 ? (
                  <Alert className='border-warning/25 bg-warning/10 text-warning'>
                    <AlertDescription className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
                      <span>
                        {t('Add')}{' '}
                        {formatModelNames(
                          props.mappingGuardrail.missingSourceModels
                        )}{' '}
                        {t(
